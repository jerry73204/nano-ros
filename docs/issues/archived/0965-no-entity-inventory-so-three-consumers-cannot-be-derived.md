...
| `NROS_EXECUTOR_ARENA_SIZE` | entities that will occupy slots | six bisections against a board | **DERIVED** 2026-09-06. `nros-node/build.rs` sums per KIND once all five `NROS_ENTITY_COUNT_*` arrive, which they now do: subscriptions at three receive buffers each, timers at 64 B, service servers at a pub/sub entry plus a reply buffer. The safety island derives 46272 against its hand-set 40960 and boots (stage 6 FirstSpin, arena 17736/46272). It still bills the triple-buffer WORST case rather than `(depth+1)*bound`, so it is conservative rather than tight -- a question about precision, not about having a derivation |
