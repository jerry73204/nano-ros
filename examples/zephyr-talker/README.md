...
```bash
west build -b qemu_cortex_m3 nano-ros/examples/zephyr-talker
west build -t run
```
...
```bash
west build -b nucleo_f429zi nano-ros/examples/zephyr-talker
west flash
```
...
cargo run -p native-listener --features zenoh -- --tcp 127.0.0.1:7447
...
- `examples/zephyr-listener/` - Subscriber example
